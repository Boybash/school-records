import * as XLSX from "xlsx";
import {
  buildResultData,
  addResult,
  checkDuplicateResult,
} from "@/lib/results";

export const downloadTemplate = (students, subjects, term, session) => {
  // Helper to identify if a class is junior or senior
  const resolveStudentCategory = (className) => {
    if (!className) return "junior";
    const upperClass = className.toUpperCase();
    if (upperClass.includes("JSS")) return "junior";
    if (upperClass.includes("SS")) return "senior";
    return "junior";
  };

  // 1. Sort the students naturally class-by-class, then alphabetically by name
  const sortedStudents = [...students].sort((a, b) => {
    const classA = a.class || "";
    const classB = b.class || "";

    // Natural alphanumeric sorting (JSS 1 -> JSS 2 -> SS 1 -> SS 2, etc.)
    const classCompare = classA.localeCompare(classB, undefined, {
      numeric: true,
      sensitivity: "base",
    });

    if (classCompare === 0) {
      return (a.name || "").localeCompare(b.name || "");
    }

    return classCompare;
  });

  const rows = [];

  // 2. Loop through each student and dynamically map eligible subjects only
  sortedStudents.forEach((student) => {
    const studentCategory = resolveStudentCategory(student.class);
    const studentDept = (student.department || "").toLowerCase();

    // Filter down the subjects that actually apply to this specific student's level and path
    const applicableSubjects = subjects.filter((subject) => {
      const subCat = (subject.category || "").toLowerCase();
      const subDept = (subject.department || "").toLowerCase();

      // Check level category matches
      const matchesCategory = subCat === "all" || subCat === studentCategory;

      // Check department streams matches
      let matchesDepartment = false;
      if (studentCategory === "junior") {
        matchesDepartment = subDept === "general";
      } else {
        matchesDepartment = subDept === "general" || subDept === studentDept;
      }

      return matchesCategory && matchesDepartment;
    });

    // Generate the spreadsheet row structure for applicable pairs
    applicableSubjects.forEach((subject) => {
      rows.push({
        StudentName: student.name,
        StudentId: student.id,
        MatricNumber: student.matricNumber,
        Class: student.class,
        SubjectName: subject.name,
        SubjectId: subject.id,
        Term: term,
        Session: session,
        CA: "",
        Exam: "",
      });
    });
  });

  // 3. Export clean worksheet
  const ws = XLSX.utils.json_to_sheet(rows);
  const wb = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(wb, ws, "Results");
  XLSX.writeFile(wb, `results_template_${term}_${session}.xlsx`);
};

export const parseResultsFile = (file) => {
  return new Promise((resolve, reject) => {
    const reader = new FileReader();
    reader.onload = (e) => {
      try {
        const data = new Uint8Array(e.target.result);
        const workbook = XLSX.read(data, { type: "array" });
        const sheet = workbook.Sheets[workbook.SheetNames[0]];
        const rows = XLSX.utils.sheet_to_json(sheet);
        resolve(rows);
      } catch (err) {
        reject(err);
      }
    };
    reader.onerror = reject;
    reader.readAsArrayBuffer(file);
  });
};

export const uploadResults = async (rows, role, uid) => {
  const errors = [];
  const successes = [];

  for (const row of rows) {
    try {
      if (
        !row.StudentId ||
        !row.SubjectId ||
        row.CA === "" ||
        row.Exam === ""
      ) {
        errors.push(`Missing data for ${row.StudentName} - ${row.SubjectName}`);
        continue;
      }

      if (Number(row.CA) > 30) {
        errors.push(
          `CA score exceeds 30 for ${row.StudentName} - ${row.SubjectName}`,
        );
        continue;
      }

      if (Number(row.Exam) > 70) {
        errors.push(
          `Exam score exceeds 70 for ${row.StudentName} - ${row.SubjectName}`,
        );
        continue;
      }

      const isDuplicate = await checkDuplicateResult(
        row.StudentId,
        row.SubjectId,
        row.Term,
        row.Session,
      );

      if (isDuplicate) {
        errors.push(
          `Duplicate: ${row.StudentName} - ${row.SubjectName} already exists for ${row.Term} ${row.Session}`,
        );
        continue;
      }

      const data = buildResultData(
        row.StudentId,
        row.SubjectId,
        row.CA,
        row.Exam,
        row.Term,
        row.Session,
        row.StudentName,
        row.SubjectName,
        role,
        uid,
      );

      await addResult(data);
      successes.push(`${row.StudentName} - ${row.SubjectName}`);
    } catch (err) {
      errors.push(`Failed to upload ${row.StudentName} - ${row.SubjectName}`);
    }
  }

  return { successes, errors };
};
