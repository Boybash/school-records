...
export default function StudentsPage() {
  ...
  // 1. FIXED: Clean combined array compiler that handles both single strings and arrays safely
  const formAvailableClasses = useMemo(() => {
    if (role === "teacher") {
      const assignedClasses = Array.isArray(userData?.classes)
        ? userData.classes
        : [];
      const teacherOf = userData?.classTeacherOf;

      let combined = [...assignedClasses];

      if (teacherOf) {
        if (Array.isArray(teacherOf)) {
          combined = [...combined, ...teacherOf];
        } else if (typeof teacherOf === "string") {
          combined.push(teacherOf);
        }
      }
    ...
    return classes;
    // Added explicit tracking to sub-properties so React registers changes instantly
  }, [role, userData?.classes, userData?.classTeacherOf, classes]);

  // 2. FIXED: Filter block updated to check both assigned classes AND form class roles
  const filteredStudents = useMemo(() => {
    return students.filter((student) => {
      ...
      if (role === "teacher") {
        // Class Boundary Check: Checks regular assigned list OR classTeacherOf single string / list items
        const inAssignedClasses = userData?.classes?.includes(student.class);
        const isFormTeacherOfClass = Array.isArray(userData?.classTeacherOf)
          ? userData.classTeacherOf.includes(student.class)
          : userData?.classTeacherOf === student.class;

        if (!inAssignedClasses && !isFormTeacherOfClass) return false;

        // Department Stream Check
  ...
  const handleSubmit = () => {
    ...
    // 3. FIXED: Allow forms creation submission validation to accept classTeacherOf assignments too
    if (role === "teacher") {
      const allowedClasses = Array.isArray(userData?.classes)
        ? userData.classes
        : [];
      const formClass = userData?.classTeacherOf;

      const isExplicitlyAllowed =
        allowedClasses.includes(trimmedClass) ||
        (Array.isArray(formClass)
          ? formClass.includes(trimmedClass)
          : formClass === trimmedClass);
  ...
  return (
    <div className="relative">
      ...
      {/* Add/Edit Form */}
      {role === "teacher" && !userData.classTeacherOf ? (
        <div className="w-full mb-6">
          <div className="bg-[#021024] rounded-md shadow p-10 text-center flex flex-col justify-center items-center">
            <img
              className="w-12 h-12 bg-white p-2 mb-2 rounded-full object-contain"
              src="/padlock.png"
              alt="padlock"
            />
            ...
            <p className="text-gray-400 text-sm">
              Only class teachers can access this Funtion. Contact admin if you
...
//   const formAvailableClasses = useMemo(() => {
//     if (role === "teacher") {
//       const assignedClasses = userData?.classes || [];
//       const classTeacherRole = userData?.classTeacherOf || [];

//       // Combine both arrays into a single list
//       const combined = [...assignedClasses, ...classTeacherRole];

//       // Remove duplicates just in case they overlap (e.g. they teach the class they manage)
