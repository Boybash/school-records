...
export default function TeacherCommentsPage() {
  ...
  useEffect(() => {
    if (
      ...
      setSelectedClass(userData.classTeacherOf);
    }
  }, [role, userData]);
  ...
  // ✅ Class teacher only sees their own class in the dropdown
  const availableClasses = useMemo(() => {
    if (role === "teacher") {
      if (userData?.isClassTeacher && userData?.classTeacherOf) {
        return [userData.classTeacherOf];
      }
      return []; // not a class teacher — no access
    }
    return [...new Set(students.map((s) => s.class))].sort();
  }, [role, userData, students]);

  // ✅ Check if current teacher is class teacher of selected class
  const isClassTeacher = useMemo(() => {
    if (role === "admin") return true;
    return (
      role === "teacher" &&
      userData?.isClassTeacher === true &&
      userData?.classTeacherOf === selectedClass
    );
  }, [role, userData, selectedClass]);

  const isPrincipalOrAdmin = role === "admin";
  ...
  const handleFieldChange = (studentId, field, text) => {
    setCommentPayloads((prev) => ({
      ...prev,
      [studentId]: { ...prev[studentId], [field]: text },
    }));
  };
  ...
  if (role === "teacher" && !userData?.isClassTeacher) {
    return (
      <div className="w-full space-y-6">
        ...
        <div className="bg-[#021024] rounded-md shadow p-10 text-center flex flex-col justify-center items-center">
          ...
          <p className="text-gray-400 text-sm">
            Only class teachers can access this page. Contact admin if you
    ...
    <div className="w-full space-y-6">
      ...
      <div className="bg-[#021024] rounded-md shadow p-6 text-white">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-black">
          {/* ✅ Class dropdown — locked for class teachers */}
          <div>
            <select
              className="border p-3 rounded-lg outline-none bg-white w-full"
              value={selectedClass}
              onChange={(e) => setSelectedClass(e.target.value)}
              disabled={role === "teacher" && userData?.isClassTeacher}
            >
        ...
        {!selectedClass ? (
          ...
          <div className="space-y-4">
            ...
            <div className="hidden md:grid md:grid-cols-12 gap-4 pb-3 border-b border-white/20 text-gray-400 font-medium text-sm px-2">
              ...
              <div className="col-span-4">Class Teacher's Comment</div>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="col-span-1 md:col-span-3">
                      <p className="font-semibold text-white text-base md:text-sm">
                        {student.name}
                      </p>
                    ...
                    <div className="col-span-1 md:col-span-4 space-y-1">
                      ...
                      <textarea
                        rows={2}
                        className="w-full p-2 rounded-md bg-white text-black text-xs outline-none focus:ring-2 focus:ring-primary-50 resize-y disabled:bg-gray-100 disabled:cursor-not-allowed"
                      ...
                      {!isClassTeacher && (
                        <span className="text-[10px] text-gray-500 block mt-1">
                          Only the class teacher can write this
                        </span>
                      )}
                    </div>
                    ...
                    {/* Save Button */}
                    <div className="col-span-1 md:col-span-1 h-full flex items-center justify-end md:justify-center">
                      <button
                        ...
                        className="w-full md:w-auto bg-gray-100 disabled:opacity-50 text-primary font-bold text-xs py-2.5 md:py-2 px-4 rounded shadow transition cursor-pointer text-center"
                      >
                        {commentMutation.isPending &&
                        commentMutation.variables?.studentId === student.id
                          ? "Saving..."
